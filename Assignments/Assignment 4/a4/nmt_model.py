...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        dec_first_hidden = torch.cat([last_hidden[0,:,:], last_hidden[1,:,:]], dim=-1)
        dec_first_hidden = self.h_projection(dec_first_hidden)
        dec_first_cell = torch.cat([last_cell[0,:,:], last_cell[1,:,:]], dim=-1)
        dec_first_cell = self.c_projection(dec_first_cell)
        dec_init_state = (dec_first_hidden, dec_first_cell)
